...
def save_videos(vid_path, save_path, vid_writer, vid_cap, save_vids_path, fourcc, image):
    ...
    if vid_path != save_path:  # new video
        ...
        fps = vid_cap.get(cv2.CAP_PROP_FPS)-10
...
while True:
    ...
    if values["-TRACK-"]:
        ...
        if values["-PRINT-"]:
            txt_path = []
            for cam in camList:
                path = out_txt + cam + '/' + str(t)
                txt_path.append(path)
                if not Path(path).exists():
                    Path(path).mkdir()
                    
            save_txt = True
            write_tracks(frame_idx1, save_txt, outputs1, txt_path[0])
            write_tracks(frame_idx2, save_txt, outputs2, txt_path[1])
            # write_tracks(frame_idx3, save_txt, outputs3, txt_path[2])
            write_tracks(frame_idx4, save_txt, outputs4, txt_path[3])
        
        # Saving Images
        if values["-SAVEIMG-"]:
            save_imgs_path = []
            for cam in camList:
                path = out_imgs + cam + '/' + str(t)
                save_imgs_path.append(path)
                if not Path(path).exists():
                    Path(path).mkdir()
            save_images(frame_idx1, outputs1, save_imgs_path[0], frame1)
            save_images(frame_idx2, outputs2, save_imgs_path[1], frame2)
            # save_images(frame_idx3, outputs3, save_imgs_path[2], frame3)
            save_images(frame_idx4, outputs4, save_imgs_path[3], frame4)
    
    # For "Recording/Saving" videos
    if values["-SAVEVID-"]:
        save_vids_path = []
        for cam in camList:
            path = out_vids + cam + '/' + str(t)
            save_vids_path.append(path)
            if not Path(path).exists():
                Path(path).mkdir()
        vid_path1, vid_writer1 = save_videos(vid_path1, save_path, vid_writer1, vs1, save_vids_path[0], fourcc, frame1)
        vid_path2, vid_writer2 = save_videos(vid_path2, save_path, vid_writer2, vs2, save_vids_path[1], fourcc, frame2)
        # vid_path3, vid_writer3 = save_videos(vid_path3, save_path, vid_writer3, vs3, save_vids_path[2], fourcc, frame3)
        vid_path4, vid_writer4 = save_videos(vid_path4, save_path, vid_writer4, vs4, save_vids_path[3], fourcc, frame4)
